...
(function($, window, document, undefined) {
  ...
  var Slider = {
    ...
    addNavigation: function(navClass) {
      ...
      if (self.options.mobileNavigation) {
        ...
        $.each(
          (self.$elem).find(self.options.panelLabelSlideSelector),
          function(n) {
      ...
      $.each(
        ...
        function(n) {
          ...
          if (!self.options.includeLabelSlide) $(this).remove();
        }
      );
    },
    ...
    registerCrossLinks: function() {
      ...
      (self.crosslinks).on('click', function(e) {
        ...
        self.setNextPanel(self.getPanelNumber(($(this).attr('href').split('#')[1]), self.options.panelLabelSlideSelector));
        e.preventDefault();
      });
      self.updateClass();
    },
    ...
    updateHashTags: function() {
      ...
      window.location.hash = self.getFromPanel(self.options.hashLabelSlideSelector, filtered);
    },
    ...
    getFirstPanel: function() {
      ...
      if (self.options.hashLinking) {
        output = self.getPanelNumber(window.location.hash, self.options.hashLabelSlideSelector);
    ...
    getPanelNumber: function(input, searchTerm) {
      ...
      (self.$panelClass).each(function(i) {
        LabelSlide = self.convertRegex($(this).find(searchTerm).text());
        if (LabelSlide === output) {
          output = i + 1;
        }
      });
    ...
    updateClass: function() {
      ...
      if (self.options.crossLinks && self.crosslinks) {
        ...
        (self.crosslinks).each(function() {
          if ($(this).attr('href') === ('#' +
            self.getFromPanel(self.options.panelLabelSlideSelector, self.sanatizeNumber(self.nextPanel) - 1))) {
  ...
  $.fn.liquidSlider.options = {
    ...
    includeLabelSlide: true,
    panelLabelSlideSelector: ".LabelSlide",
    ...
    crossLinks: false,
    hashLinking: false,
    hashLabelSlideSelector: ".LabelSlide",
